...
class GaussianND : public Gaussian
{
public :
   ...
   */
  GaussianND(string nomfic, string dispdec=""); 
  
  /*! \fn Gaussian(int dim)
   *  \brief Class constructor
   *  \param dim the gaussian dimension
   */
  GaussianND(int dim, string dispdec=""); 
   ...
   */
  GaussianND(int dim, double* m, double* c, string dispdec="");
  
  /*! \fn Gaussian* Clone()
   *  \brief this method create a new gaussian identical to this one and return
   *  a pointer on this cloned gausssian
   */
  inline GaussianND* Clone() 
  { return new GaussianND(this->dim,this->mean,this->cov,this->dispdec); };
