...
public :
   ...
   */
  Gaussian3D(string nomfic, string dispdec=""); 
  
  /*! \fn Gaussian(int dim)
   *  \brief Class constructor
   *  \param dim the gaussian dimension
   */
  Gaussian3D(int dim, string dispdec=""); 
   ...
   */
  Gaussian3D(int dim, double* m, double* c, string dispdec="");
  
  /*! \fn Gaussian* Clone()
   *  \brief this method create a new gaussian identical to this one and return
   *  a pointer on this cloned gausssian
   */
  inline Gaussian3D* Clone() 
  { return new Gaussian3D(this->dim,this->mean,this->cov,this->dispdec); };
