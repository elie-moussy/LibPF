...
public :
   ...
   */
  Gaussian2D(string nomfic, string dispdec=""); 
  
  /*! \fn Gaussian(int dim)
   *  \brief Class constructor
   *  \param dim the gaussian dimension
   */
  Gaussian2D(int dim, string dispdec=""); 
   ...
   */
  Gaussian2D(int dim, double* m, double* c, string dispdec="");
  
  /*! \fn Gaussian* Clone()
   *  \brief this method create a new gaussian identical to this one and return
   *  a pointer on this cloned gausssian
   */
  inline Gaussian2D* Clone() 
  { return new Gaussian2D(this->dim,this->mean,this->cov,this->dispdec); };
