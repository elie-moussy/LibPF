#ifndef GAUSSIANDIAG3D_H
#define GAUSSIANDIAG3D_H

#include "LibPF/distribution/Gaussian.h"

class GaussianDiag3D : public Gaussian
{
public :
   ...
   */
  GaussianDiag3D(string nomfic, string dispdec=""); 
  
  /*! \fn Gaussian(int dim)
   *  \brief Class constructor
   *  \param dim the gaussian dimension
   */
  GaussianDiag3D(int dim, string dispdec=""); 
   ...
   */
  GaussianDiag3D(int dim, double* m, double* c, string dispdec="");
  
  /*! \fn Gaussian* Clone()
   *  \brief this method create a new gaussian identical to this one and return
   *  a pointer on this cloned gausssian
   */
  inline GaussianDiag3D* Clone() 
  { return new GaussianDiag3D(this->dim,this->mean,this->cov,this->dispdec); };
