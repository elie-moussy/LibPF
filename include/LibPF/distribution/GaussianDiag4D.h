#ifndef GAUSSIANDIAG4D_H
#define GAUSSIANDIAG4D_H

#include "LibPF/distribution/Gaussian.h"

class GaussianDiag4D : public Gaussian
{
public :
   ...
   */
  GaussianDiag4D(string nomfic, string dispdec=""); 
  
  /*! \fn Gaussian(int dim)
   *  \brief Class constructor
   *  \param dim the gaussian dimension
   */
  GaussianDiag4D(int dim, string dispdec=""); 
   ...
   */
  GaussianDiag4D(int dim, double* m, double* c, string dispdec="");
  
  /*! \fn Gaussian* Clone()
   *  \brief this method create a new gaussian identical to this one and return
   *  a pointer on this cloned gausssian
   */
  inline GaussianDiag4D* Clone() 
  { return new GaussianDiag4D(this->dim,this->mean,this->cov,this->dispdec); };
