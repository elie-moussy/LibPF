#ifndef GAUSSIANDIAGND_H
#define GAUSSIANDIAGND_H

#include "LibPF/distribution/Gaussian.h"

class GaussianDiagND : public Gaussian
{
public :
   ...
   */
  GaussianDiagND(string nomfic, string dispdec=""); 
  
  /*! \fn Gaussian(int dim)
   *  \brief Class constructor
   *  \param dim the gaussian dimension
   */
  GaussianDiagND(int dim, string dispdec=""); 
   ...
   */
  GaussianDiagND(int dim, double* m, double* c, string dispdec="");
  
  /*! \fn Gaussian* Clone()
   *  \brief this method create a new gaussian identical to this one and return
   *  a pointer on this cloned gausssian
   */
  inline GaussianDiagND* Clone() 
  { return new GaussianDiagND(this->dim,this->mean,this->cov,this->dispdec); };
