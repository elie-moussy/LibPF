...
class Gaussian4D : public Gaussian
{
public :
   ...
   */
  Gaussian4D(string nomfic, string dispdec=""); 
  
  /*! \fn Gaussian(int dim)
   *  \brief Class constructor
   *  \param dim the gaussian dimension
   */
  Gaussian4D(int dim, string dispdec=""); 
   ...
   */
  Gaussian4D(int dim, double* m, double* c, string dispdec="");
  
  /*! \fn Gaussian* Clone()
   *  \brief this method create a new gaussian identical to this one and return
   *  a pointer on this cloned gausssian
   */
  inline Gaussian4D* Clone() 
  { return new Gaussian4D(this->dim,this->mean,this->cov,this->dispdec); };
